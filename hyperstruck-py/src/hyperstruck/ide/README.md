...
One thing beyond the name travels from a failed step: the name the failure itself carried,
as a token such as `ModuleNotFoundError` or as a masked template such as `File has not been
read yet`. That is what lets a learned rule carry a *condition* rather than firing on every
use of a tool.

**What keeps such a token safe is where it is read from, never what it looks like.** The
shape it must match is equally the shape of a username, a hostname and an environment
variable name, so a rule taking the leading token of a line ships those, and every rule
built on position did: reading the last line of a result, reading the line after the host's
framing, reading a field that holds nothing but a message, and reading the framed line
itself. Measured over 781 real failures, the most common value the last of those derived was
`root`, a fragment of a JSON-schema path. No content rule can separate them either, because
`ModuleNotFoundError: No module named x` and `tlynch: permission denied` are the same string
shape.

So position was replaced by **provenance**: a grammar that names the author of the text
before anything is read out of it. Three tiers follow, and the third is the load-bearing one.

```
  the host's own protocol message   ->  masked template   "File has not been read yet"
  a language runtime's error name   ->  the class name    "ModuleNotFoundError"
  the user's program                ->  nothing
```

A host composes its messages from a fixed vocabulary, so what varies inside one is a path or
a value, is replaced by a mask, and what remains recurs across machines. A language runtime's
error name is read under that language's published error format, and what is taken is the
class-name-shaped identifier and never the message, so `KeyError: 'secret-tenant-id'` yields
`KeyError`. Everything else abstains: when a frame consumes the whole line, the next line is
the program's own output, and there is no lower tier that takes a token from an unlicensed
line.

Two consequences are worth stating plainly. A generic envelope like `Error:` or `Exit code 1`
licenses **nothing**, because what follows it is whatever the program printed; masking cannot
rescue it, since a hostname and a username both satisfy the boundary's own word pattern and
survive masking intact. And a field named `stderr` licenses nothing either: a field name is a
claim about the field, not about each line inside it.

**A block is recognised, never a line.** The licence is a banner, and what it licenses is the
contiguous run of frames beneath it plus the one unindented line that ends the run. Only
CPython qualifies today. V8 was declared and withdrawn: its format opens with the error name
and has *no banner*, so the only licence available was an indented `at ` line somewhere below,
which a Java stack trace also satisfies and which says nothing about who wrote the line above
it. A language with no banner has no recogniser, and gets no entry rather than a positional
rule wearing a grammar's name.

**What the tier reads and what the boundary keeps are not the same set.** The boundary bounds
an operand by entropy per character, and a *qualified* exception name exceeds it: the dots and
the case changes that make `asyncio.TimeoutError` more readable are exactly what push it over.
The client mirrors that bound rather than sending a value to be dropped silently on arrival, so
those names are read correctly and then refused, and the debug channel says so in those words
rather than reporting that no tier licensed the failure. Entropy per character is the wrong
instrument for a dotted name; correcting it is Core's work, not the client's.

**A host frame does not mean the host composed everything after it.** Claude Code's own
protocol messages sometimes interpolate the user's verbatim command. So the word rule the
masking applies is tighter than the boundary's: a word carrying a dot or an underscore is
masked as an identifier rather than kept as prose.

**That narrows the exposure and does not remove it, and it is worth being exact about why.**
An undotted bare token is the same shape as an ordinary word, and as a class name: `bastion`
against `blocked`, `tlynch` against `KeyError`. Three separate rules have been tried here and
each closed the cases it was shown and left the class open. Safety is not a property of the
string; it is a property of the population. `KeyError` occurs across thousands of runs and many
tenants, a username occurs in one operator's.

**The boundary is what bounds it**, through a k-anonymity floor: an operand is frozen onto a
rule only after five distinct runs produce it, and the rule stays agent-specific until twenty
distinct *tenants* have produced it. A value belonging to one operator never clears the second
and is never shared. Tenants rather than agents, because delegation mints a child agent per
delegation under the same organisation, so an agent-counted floor is cleared by one customer
delegating to itself; the boundary moved the unit for that reason and this client reads which
unit is in force from the vendored contract rather than assuming. It is stored either way, and
closing that is Core's work rather than the client's.

The bounds the masking applies are read from the vendored contract at use rather than copied
into the client, because the boundary's support floors count distinct runs of a
byte-identical value, and a bound that drifted by one character would stop them accumulating
at all. A failed step's `error` field still carries the host's own error text, clipped and
scrubbed, under the same rules as before.
...
**Declared contrast** decides which shelf the corpus lands on, not whether it is sent.
With it the learning extractor has something to compare and can mine a rule; without it
the corpus still carries facts, and those reach the claim shelf. The client sends both,
tells you at send time which one it just handed over, and refuses only an empty corpus or
one outside the boundary's size bounds, which it checks here so the reason survives.
Use it for corpus text only: a real run trace is the automatic loop, and a final learning
you already have verbatim belongs in the curation API.
...
The gate operand names its abstentions the same way, and the distinctions matter
because they look identical from outside and mean opposite things: no provenance
tier licensed the failure (the common case, and by design), a tier read a name
that the boundary's published bounds then refused (the tier working and the
contract disagreeing, which is what a qualified exception name hits), the
scrubber recognising a credential in a licensed value, or the boundary
publishing no support-floor guarantee at all (the whole lane being off). Only
the last needs action, and not in your configuration: nothing you can set moves
it, because it is read from the contract vendored inside the wheel. A stale or
damaged install is the cause, so reinstall, and report it if it survives that. The same switch covers the
framework-neutral layer, whose lines are tagged `[hyperstruck]` rather than
`[hyperstruck-hook]`; a vendored contract that cannot be read degrades to empty
and says so there. Debugging is off by default, so
