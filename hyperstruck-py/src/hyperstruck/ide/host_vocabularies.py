...
# emits. Removed before the failure's own name is read, because Claude Code leads every
# failed result with ``Error:`` and an ``Exit code N`` line, and leaving them makes the
# operand ``Error`` for 65% of all failures: the degenerate gate reached by another route.
_SOURCE_FAILURE_FRAMING = {
    SOURCE_CLAUDE_CODE: (r"^Error\s*:?\s*", r"^Exit code \d+\s*:?\s*"),
}


def failure_framing(source: str) -> tuple[str, ...]:
