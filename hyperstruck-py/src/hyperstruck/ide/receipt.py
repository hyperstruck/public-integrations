...
def acceptance_record(
    ...
    # TODO(ceiling): this answer is now load-bearing in a way it was not when it only
    # chose a diagnostic. NO_MATCHING_RECORD reports the recall as never delivered, so a
    # line still being flushed when this read runs, or a marked line that stopped
    # parsing, writes that claim rather than merely mislabelling a delivered run. The
    # boundary's delivery latch only moves toward delivered, so a later correct report
    # can raise it but nothing lowers a wrong one. Revisit if the refusal bucket ever
    # carries volume the editor cannot account for: the fix is a second read on the
    # next turn, not a wider outcome set.
