...
- **Requirements**: 1 to 50 evidence items, each at most 8,000 characters and
  120,000 across the corpus. The command checks those bounds itself, so an oversized
  corpus is refused here with the reason rather than remotely as a bare status code.
...
- **Result**: extraction runs server-side and is asynchronous; the command reports
  ...
  reusable contrast, reported as `nothing_extracted`. A zero yield is **not free**:
