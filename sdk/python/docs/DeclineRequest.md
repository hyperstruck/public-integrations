...
**recall_outcome** | **object** | Why the recall did not reach the model, when it did not, from the same closed set reinforce uses. A declined turn that was never shown its learnings is a different run from one that was shown them and did too little to be worth learning from. Sending this without &#x60;is_delivered&#x60; is enough: the flag is derived from it. Sending both with different meanings is refused. | [optional]
